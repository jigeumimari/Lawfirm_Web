// script_dashboard_patch.js — loads metrics ONLY when you navigate to "dashboard"
(function () {
  ...
  async function refreshDashboard(){
    // If Dashboard route isn't active, do nothing
    ...
    } catch(e){ setTxt('#metricOpenCases', '—'); }
    ...
    }catch(e){ setTxt('#metricUpcomingEvents', '—'); }

    // Unread messages (best-effort)
    try{
  ...
  // Hook into route changes ONLY; no auto-run on load
  window.addEventListener('DOMContentLoaded', () => {
    if (typeof routeTo === 'function') {
      const __routeTo = routeTo;
  ...
  // Expose for manual calls if you ever want to refresh after actions
