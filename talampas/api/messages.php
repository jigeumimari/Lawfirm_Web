<?php
// api/messages.php
// Messages within a thread. No auto "hello" seed.
// Tables: messages(id, thread_id, sender_id, body, created_at)
...
// permission helper: is user in this thread?
...
try {
  if ($method === 'GET') {
    // List messages for a thread: ?thread_id=123
  ...
  if ($method === 'POST') {
    // Create a message
    // Input: { thread_id: number, body: string }
    $in = $_POST ?: json_input();
  ...
  if ($method === 'DELETE') {
    // Delete a message: user can delete their own; staff can delete any in their threads
