...
function to_time_his(string $v): string {
  $v = trim($v);
  // Let PHP handle "04:22 PM" etc.
  $dt = date_create($v);
  if ($dt) return $dt->format('H:i:s');
  // Fallback: HH:MM to HH:MM:SS
...
try {
  if ($method === 'GET') {
    // List events by date range (YYYY-MM-DD)
    $from = $_GET['from'] ?? null;
    $to   = $_GET['to']   ?? null;
  ...
  if ($method === 'POST') {
    ...
    // Accept either start_datetime OR event_date + event_time
    $event_date = null;
    $event_time = null;

    if (!empty($in['start_datetime'])) {
      $raw = str_replace('/', '-', (string)$in['start_datetime']);     // allow 09/16/2025 04:22 PM
      $dt  = date_create($raw);
