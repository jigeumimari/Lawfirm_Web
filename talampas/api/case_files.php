<?php
// api/case_files.php (fixed to match schema)
// tables: case_files(id, case_id, file_name, file_path, uploaded_by, created_at)
// This endpoint handles: GET (list by case_id), POST (upload metadata only; you can combine with actual upload handler),
// DELETE by id. Uses PDO and the auth model used elsewhere.
...
try {
    if ($method === 'GET') {
        ...
        // clients can access files only for their own case
        if ($user['role'] === 'client') {
    ...
    if ($method === 'POST') {
        // metadata insert; assumes an upload handler already stored file to disk and provided file_name/file_path
        $case_id = (int)($_POST['case_id'] ?? 0);
        $file_name = $_POST['file_name'] ?? '';
        $file_path = $_POST['file_path'] ?? '';
        if (!$case_id || $file_name === '' || $file_path === '') bad(422, 'case_id, file_name, file_path required');

        // permission: client may add files only to their own case
        if ($user['role'] === 'client') {
            $chk = $pdo->prepare("SELECT client_id FROM cases WHERE id=?");
            $chk->execute([$case_id]);
            $row = $chk->fetch(PDO::FETCH_ASSOC);
