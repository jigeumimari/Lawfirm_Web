...
/* ========= Forgot Password (API) ========= */
/* POST api/reset_password.php  { email, new_password } */
async function resetPassword(email, new_password){
  const res = await api('reset_password.php', {
    method:'POST',
...
function enterApp(user){
  ...
  function refreshAppointments(){ /* no-op */ }
}
    ...
    <tr data-id="${c.id}">
      ...
      <td class="actions">
        <div class="case-actions">
          <button type="button" class="btn-edit"   data-action="update">Edit</button>
...
document.addEventListener('click', async (e) => {
  const btn = e.target.closest('#caseTable [data-action]');
  if (!btn) return;
  ...
  if (act === 'delete') {
    ...
    try {
      await api(`cases.php?id=${encodeURIComponent(id)}`, { method: 'DELETE' });
      toast('Case deleted');
      refreshCases();
    } catch (err) {
      console.error(err);
    ...
    return; // ← return belongs here, after the catch
  }

  if (act === 'update') {
    const curTitle = tr.children[2]?.textContent?.trim() || '';
    const newTitle = prompt('Edit title:', curTitle);
    if (newTitle === null) return;
    try {
      await api(`cases.php?id=${encodeURIComponent(id)}`, {
        method: 'PATCH',
...
async function submitAppointment(e){
  ...
  await refreshEvents();   // show it instantly on the calendar
}
}
...
// Normalize calendar_events rows and appointment rows into one shape
function normalizeCalItem(x){
  if (x.start_datetime) {
    return {
      id: x.id,
      title: x.title || '—',
      ...
      isAppointment: (x.type === 'appointment') || false,
      status: x.status || null
    };
  }
  const date = (x.event_date || '').toString().slice(0,10);
  let time   = (x.event_time || '').toString().slice(0,8);
  if (!time && x.start_time) time = x.start_time.toString().slice(0,8);
  const start = `${date}${date ? ' ' : ''}${time || ''}`.trim();
...
async function submitEvent(e){
  ...
  const date  = (q('#eventDate')?.value || '').trim();
  const time  = (q('#eventTime')?.value || '').trim();
...
const Availability = (() => {
  ...
  async function fetchList(){
    const res = await api('availability.php');
    const arr = Array.isArray(res) ? res : (res.items || res.data || []);
    return arr.map(o => ({
      id:      o.id ?? o.availability_id ?? o.avail_id,
      user:    o.user_name ?? o.full_name ?? o.name ?? '—',
      date:    (o.date || o.avail_date || '').slice(0,10),
      from:    (o.from || o.from_time || '').slice(0,5),
      to:      (o.to || o.to_time || '').slice(0,5),
  ...
  async function render(){
    ...
    try{
      ...
      if (!rows.length){
        tbody.innerHTML = `<tr><td colspan="6" class="hint">No availability yet.</td></tr>`;
        return;
      }
      tbody.innerHTML = rows.map(r => `
        <tr data-id="${r.id||''}">
          ...
          <td>${r.to||'—'}</td>
          <td>
            <span class="badge ${/unavail/i.test(r.status)?'unavail':'avail'}">${r.status}</span>
            ${r.notes ? ` · <span class="hint">${r.notes}</span>` : ''}
          </td>
          <td class="only-admin"></td>
        </tr>
      `).join('');
    }catch(e){
      console.error('[Availability] load failed:', e);
      tbody.innerHTML = `<tr><td colspan="6">Failed to load availability.</td></tr>`;
    }
  }
...
function onRouteEnter(name){
  ...
  if (name==='availability') Availability.render();
}
...
window.addEventListener('DOMContentLoaded', async () => {
  ...
  on(q('#forgotBtn'), 'click', () => {
    q('#forgotForm')?.reset();
    const currentEmail = q('#email')?.value?.trim() || '';
    if (currentEmail) q('#fpEmail').value = currentEmail;
    const dlg = q('#forgotDialog');
    if (dlg && typeof dlg.showModal === 'function') dlg.showModal(); else dlg?.setAttribute?.('open','');
  });
  on(q('#fpCancelBtn'), 'click', () => q('#forgotDialog')?.close?.());
  on(q('#forgotForm'), 'submit', async (e) => {
    e.preventDefault();
    const email = (q('#fpEmail')?.value || '').trim().toLowerCase();
    const newPass = q('#fpNew')?.value || '';
    if (!email || !newPass) return toast('Enter email and new password');
    try {
  ...
  // If Availability is already the active view on load, render once.
  const active = q('.route.active') || q('#route-cases') || q('#route-dashboard');
  if (active && active.id === 'route-availability') {
    Availability.render();
  }
});
...
// Update prefix automatically when a country is selected
phoneCountry.addEventListener('change', () => {
  phonePrefix.textContent = phoneCountry.value;
});
